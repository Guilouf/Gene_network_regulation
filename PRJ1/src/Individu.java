...
//////////////////////////////////////RANDOM//////////////////////CMPLX: taillegeno?
    ...
    public boolean testVia() {
        /**
         * Return a boolean, if a matrix is viable, i.e
         */

        boolean viable = false ;
        List<Matrix> listmat  = new LinkedList<Matrix>(); //On peut laisser ca, linkedlist est bien adapté pour la situation.

        ////Construction du premier vecteur
        Matrix seu = Outils.vecteurDeDEpart(1, taillegeno); //start vector full of 1
        Matrix genotypeAtester = this.genotype; // matrix to test

        ////////////////grande boucle///////////////////////
        boolean cond = false; //condition de sortie de boucle, dès que l'on sait si le vecteur est viable ou non.
        int tour = 0;
        ...
        while ( cond == false ) {	//Ya un compteur
            ...
            listmat.add(seu) ; //add the precedent vector to the list

            /////seuillage du vecteur
            Outils.seuillerVecteur(vecteur, taillegeno);

            ////return true if the matrix are identical: check between the precedent and the actual vector
            boolean condI =  Outils.comparerVecteur(taillegeno, listmat.get(tour), vecteur) ;

            if (condI == true) {	//le vecteur est viable
                cond = true ;
                viable = true ;
                this.spdvia = tour ; // record the number of turn necessary to get a stable matrix
                return viable ; //c'est ici le trick.. la boucle s'arrete la, et le reste de la fct aussi.
            }

            //// test non viable
            int k = 0 ;
            // check if there was an identical vector in the historic, if there was, the matrix is non viable, because it loops
            while ( k < tour ) {		// k < tour  pour parcourir la liste (tour représente la taille de listmat en gros) => C'est mauvais
                // todo refaire avec une collection
                k++;
...
///////////////////////////MUTATION////////////////////////////////////////
    ...
    ///////////////////////////KO////////////////////////////////////////
    public void KO(double seuilKO, int nbKO) { //KO aléatoire
        Random ran = new Random();
    ...
    ///////////////////////////Compteur lignes/////////////////
    public double comptLigne(int index) { //todo other to identify the line? memmory adress, hash?
        /**
         * return the sum of a matrix line. The sum is supposed to statistically unique, thus representing as unique
    ...
    ////////////////////////COMPTEUR GENE
    public double comptGeneUni(int index) {
        /**
         * return the sum of a matrix line + matrix column, always from a diagonal coefficient representing a gene
         * The sum is supposed to statistically unique, thus representing as unique combination of coefficients
         */
        double valgene = 0 ;
        for (int j = 0 ; j < taillegeno ; j++ ) {
            valgene += this.genotype.get(index, j) ; //sum of a line
        }
        for ( int k = 0 ; k < taillegeno ; k++) {
            valgene += this.genotype.get(k, index ) ;// sum of column
        }
        return valgene ;
    }
